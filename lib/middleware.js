...
module.exports = function (app) {
  ...
  const setRenderObject = function (req, res, next) {
    // Pyt the query string in the render object so it's available to templates
    res.locals.render = res.locals.render || {};
    res.locals.render = { ...res.locals.render, query: req.query };
    return next();
  };
  ...
  const serveEverything = async function (req, res, next) {
    ...
    if (foundFile && routing.noDotsInPath(searchPath)) {
      console.debug(`ℹ️  (serveEverything) Found file:`);
  ...
  const serveEverythingFrom = function (appFolder) {
    return async function (req, res, next) {
      let searchExtensions = [];
      if (req.path === "/") {
        searchExtensions = ["index.{js,pug,html}"];
      } else {
        searchExtensions = [
          path.join(req.path, "index.{js,pug,html}"),
          `${req.path}.{js,pug,html}`,
        ];
      }

      for (const searchExtension of searchExtensions) {
        const foundFile = await routing.findFirst(path.join(appFolder, searchExtension));
        if (foundFile && routing.noDotsInPath(path.join(appFolder, searchExtension))) {
          console.debug(`ℹ️  (serveEverythingFrom) Found file:`);
          console.dir(foundFile);
          res.locals.render.template = `${req.path.replace("/", "")}`;
          res.locals.render.filePath = foundFile;
          res.locals.render.fileExtension = path.extname(foundFile);
  ...
  const defaultRender = function (req, res, next) {
    app.locals.debug &&
      console.debug(`ℹ️  (defaultRender) Rendering: ${res.locals.render?.template || "index"}`);
    if (res.locals.isFoundRoute) {
      console.debug(`===> File extension: ${res.locals.render?.fileExtension}`);
